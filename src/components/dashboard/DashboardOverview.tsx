  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard title="ยอดรวมคงเหลือ" value={balance.balance.toLocaleString()} subtitle="รวมรายรับ-รายจ่าย" tone="primary" href="/transactions" />
        <StatCard title="รายรับ (ธุรกรรม)" value={balance.incomeTxn.toLocaleString()} subtitle="ไม่นับการเก็บนักเรียน" tone="success" href="/transactions?source=transaction&kind=income" />
        <StatCard title="รายจ่าย (ธุรกรรม)" value={balance.expenseTxn.toLocaleString()} subtitle="ทั้งหมด" tone="danger" href="/transactions?kind=expense" />
        <StatCard title="รายรับจากการเก็บ" value={balance.studentIncome.total.toLocaleString()} subtitle="ธนาคาร/เงินสด/TrueMoney" href="/transactions?source=schedule" />
      </div>
      ...
      <div className="rounded-xl border p-4">
        ...
        {hasSchedules ? (
          <div className="grid gap-4 sm:grid-cols-2">
            <Link href={`/schedule?scheduleId=${selectedScheduleId}&status=paid`} className="rounded-lg border bg-emerald-50/50 p-4 dark:bg-emerald-950/20 hover:border-emerald-200 dark:hover:border-emerald-800 transition-colors">
            ...
            <Link href={`/schedule?scheduleId=${selectedScheduleId}&status=unpaid`} className="rounded-lg border bg-rose-50/50 p-4 dark:bg-rose-950/20 hover:border-rose-200 dark:hover:border-rose-800 transition-colors">
          ...
          <div className="flex flex-col md:flex-row gap-6">
            <div className="h-64 flex-1">
              ...
              <div className="flex justify-center gap-3 mt-2 flex-wrap">
                {graphData.map((entry: { name: string; value: number }, index: number) => (
                  <Link key={entry.name} href={entry.name === "รายจ่าย" ? "/transactions" : "/schedule"} className="flex items-center gap-1.5 text-xs hover:bg-zinc-100 dark:hover:bg-zinc-800 px-1.5 py-0.5 rounded transition-colors">
            ...
            <div className="flex-1 flex flex-col justify-center gap-4 text-sm">
              <Link href="/transactions" className="p-3 bg-zinc-50 dark:bg-zinc-800 rounded-lg border dark:border-zinc-700 hover:border-zinc-400 dark:hover:border-zinc-600 transition-colors">
...
function StatCard({ title, value, subtitle, tone, href }: { title: string; value: string | number; subtitle?: string; tone?: "primary" | "success" | "danger"; href?: string; }) {
  ...
  const content = (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}
      className={cn(
        "rounded-xl border p-4 bg-gradient-to-br to-transparent h-full transition-all",
        toneClass,
        href && "hover:border-zinc-400 dark:hover:border-zinc-500 hover:shadow-sm cursor-pointer active:scale-95"
      )}>
