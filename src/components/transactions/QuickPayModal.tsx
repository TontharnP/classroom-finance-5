...
export function QuickPayModal({ isOpen, onClose, scheduleId, studentId }: Props) {
  ...
  useEffect(() => {
    if (isOpen && remaining > 0 && rows.length === 1 && rows[0].amount === 0) {
      setRows([{ method: "cash", amount: remaining }]);
    }
  }, [isOpen, remaining]);
          ...
          {rows.map((row, idx) => (
            <div key={idx} className="grid grid-cols-12 gap-2">
              ...
              {row.method === "kplus" && row.amount > 0 && (
                <div className="col-span-12 flex flex-col items-center justify-center p-4 bg-white rounded-lg border my-2">
                  <QRCodeCanvas
