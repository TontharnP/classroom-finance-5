...
export function ScheduleCalendar({ onScheduleClick }: ScheduleCalendarProps) {
  ...
  const hasScheduleOnDate = (date: Date) => {
    return orderedSchedules.some((schedule) => {
      const startDate = parseISO(schedule.startDate);
      const endDate = schedule.endDate ? parseISO(schedule.endDate) : null;
        ...
        <Calendar
          ...
          tileContent={({ date, view }) => {
            ...
            data.transactions.forEach(t => {
              if (t.createdAt.startsWith(dayStr)) {
            ...
            return (
              <div className="flex flex-col items-center text-[10px] sm:text-xs leading-none mt-0.5 gap-0.5 font-medium">
                {income > 0 && <span className="text-emerald-600 dark:text-emerald-400">+{income.toLocaleString()}</span>}
                {expense > 0 && <span className="text-rose-600 dark:text-rose-400">-{expense.toLocaleString()}</span>}
              </div>
            );
          }}
        />
